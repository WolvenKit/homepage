<script lang="ts">
  ...
  function update() {
    ...
    if (Math.random() < cleanChance) {
      ...
      pageImage.data.fill(0, start, start + length);
      resetChance -= resetChanceMultiplier * (length / pageImage.data.length);
    }
    // Otherwise place a block of content
      ...
      const width = Math.floor(Math.random() ** 0.5 * maxSideLength * (1 - shapeRatio));
      const height = Math.floor(Math.random() ** 0.5 * maxSideLength * shapeRatio);
      ...
      const background = Math.random() < 0.5 ? backgroundColor : [0, 0, 0];
        ...
        for (let x = 0; x < width; x++) {
          for (let z = 0; z < 4; z++) {
            ...
            if (replaceBackground && pageImage.data[sourceI + backgroundOffset] == 0) {
              pageImage.data[sourceI + backgroundOffset] = background[z] ?? 255;
            }
      ...
      resetChance += resetChanceMultiplier * ((width * height) / pageImage.data.length);
    }
